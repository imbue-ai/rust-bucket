...
- **Bead IDs**: When creating beads, you MUST supply a randomly generated 6-character UUID as the bead ID (e.g. `br create --id "$(openssl rand -hex 3)" ...`). Do NOT use autogenerated sequential numbers. This prevents branch name conflicts when multiple agents work across repos.
