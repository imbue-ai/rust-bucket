...
- **NEVER use the SQLite database directly.** This project runs in JSONL-only mode (`no-db: true` in `.beads/config.yaml`). All task data lives in `.beads/issues.jsonl`. Do not create, query, or reference `beads.db`.
- Ensure `.beads/.br_history/` is listed in `.gitignore`. This directory contains local backup history and must not be committed.
...
2. **Command migration:** Replace `bd` with `br`. Subcommands are the same:
   ...
   - `bd list` -> `br list`
   - `bd close` -> `br close`
3. **Data migration:** If the repo has an existing `.beads/issues.jsonl` from the old `bd` tool, beads_rust will read it directly in JSONL-only mode. No import step is needed.
4. **Verification:** Confirm all issues loaded correctly:
   ```bash
   br list
   ```
